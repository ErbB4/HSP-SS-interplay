...
# growth curves
gc_axon  = {'growth_curve': growth_curve_d, 'z': z0_mean, 'growth_rate': growth_factor[i]*slope*target_rate, 'eps': target_rate, 'eta':0.0, 'continuous': False}
gc_den = {'growth_curve': growth_curve_a, 'z': z0_mean, 'growth_rate': growth_factor[i]*slope*target_rate, 'eps': target_rate, 'eta':0.0, 'continuous': False}
...
poisson_generator_lesion = nest.Create('poisson_generator')
nest.SetStatus(poisson_generator_lesion, {"rate": rate,"stop":growth_time})

#nest.Connect(poisson_generator_lesion, pop_exc[0:N_lesion],'all_to_all', syn_spec = "device")
...
def simulate_cicle(growth_steps, step_size, recording_duration):
    ...
    for simulation_time in growth_steps:
        ...
        #syn_elems_l = nest.GetStatus(pop_exc[0:N_lesion], 'synaptic_elements')
        #spines = sum(neuron['Den_exc']['z_connected'] for neuron in syn_elems_l)
        #np.save("spines_lesion"+extension,spines)

        #syn_elems_e = nest.GetStatus(pop_exc[N_lesion::], 'synaptic_elements')
        #spines = sum(neuron['Den_exc']['z_connected'] for neuron in syn_elems_e)
        #np.save("spines_rest"+extension,spines)
        
        sampling_time_points.append(simulation_time + step_size)
        np.save(T+"/data/sampling_time_points.npy",sampling_time_points)
        count += 1
            
            
# grow network
growth_steps = np.arange(0, 2*growth_time,growth_step)
