...
parrot_to_lesion = parrots[0]
parrot_to_rest   = parrots[1]

nest.Connect(poisson_generator_ex, parrots,'all_to_all', syn_spec = "device")
nest.Connect(parrot_to_lesion, pop_exc[0:N_lesion],'all_to_all', syn_spec = "device")
nest.Connect(parrot_to_rest, pop_exc[N_lesion::],'all_to_all', syn_spec = "device")
...
def simulate_cicle(growth_steps, step_size, recording_duration):
    ...
    for simulation_time in growth_steps:
        ...
        np.save("sampling_time_points.npy",sampling_time_points)
        count += 1
...
conn = nest.GetConnections(parrot_to_rest,pop_exc[0:N_lesion])
weights_to_be_mute = nest.GetStatus(conn,"weights")
nest.SetStatus(conn,"weights")
