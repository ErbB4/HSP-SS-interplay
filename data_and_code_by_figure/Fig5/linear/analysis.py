...
for counter, time_points in enumerate(sampling_time_points):
    ...
    connectivity_ss.append(np.mean(connectivity_matrix[0:N_sub,0:N_sub]))
    connectivity_sr.append(np.mean(connectivity_matrix[0:N_sub,N_sub::]))
    connectivity_rs.append(np.mean(connectivity_matrix[N_sub::,0:N_sub]))
    connectivity_rr.append(np.mean(connectivity_matrix[N_sub::,N_sub::]))
    connectivity.append(np.mean(connectivity_matrix))

    np.save(T+"rates_all_seed_"+str(i)+".npy",rates_all)
    np.save(T+"connectivity_ss_seed_"+str(i)+".npy",connectivity_ss)
    np.save(T+"connectivity_sr_seed_"+str(i)+".npy",connectivity_sr)
    np.save(T+"connectivity_rs_seed_"+str(i)+".npy",connectivity_rs)
