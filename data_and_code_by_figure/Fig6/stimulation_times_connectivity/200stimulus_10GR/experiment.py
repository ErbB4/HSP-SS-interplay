...
nest.Connect(poisson_generator_ex, pop_exc[N_lesion::],'all_to_all', syn_spec = "device")
...
lesion_steps_long = np.arange(growth_time + growth_step, growth_time + 5*growth_step, growth_step)
simulate_cicle(lesion_steps_long, growth_step, long_duration)


poisson_generator_lesion = nest.Create('poisson_generator')
nest.SetStatus(poisson_generator_lesion, {"rate": 2.0*rate, "start":growth_time + 5*growth_step})
nest.Connect(poisson_generator_lesion, pop_exc[0:N_lesion],'all_to_all', syn_spec = "device")

opto_steps = np.arange(growth_time + 5*growth_step, growth_time + 6*growth_step, lesion_step)
simulate_cicle(opto_steps, lesion_step, short_duration)

opto_steps_long = np.arange(growth_time + 6*growth_step, growth_time + 12*growth_step, growth_step)
simulate_cicle(opto_steps_long, growth_step, long_duration)
