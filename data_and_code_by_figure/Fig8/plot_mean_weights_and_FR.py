...
############### define canvas ###########
fig = plt.figure(figsize=(cm2inch(5.3), cm2inch(7.5)))
gs1 = gridspec.GridSpec(8, 1)
gs1.update(top=0.9,bottom=0.1,left=0.2,right=0.99,hspace=0.2,wspace=0.2)
ax1 = plt.subplot(gs1[0:3,0]) #plot FR of two sample neurons
ax2 = plt.subplot(gs1[3:5,0]) # plot weights of neuron 1
ax3 = plt.subplot(gs1[6:8,0]) # plot weights of neuron 2


################## plot FR for neuron 1 and neuron 2 ##############


ax1.plot(sampling_time_points/1000.,rates1,color='#666666',label='N1 (deprived)')
ax1.plot(sampling_time_points/1000.,rates2,color='#47abd8',label='N2 (non-deprived)')
ax1.plot(sampling_time_points/1000.,[7.9]*len(sampling_time_points),'--',color='grey',linewidth=0.5)

################## plot weights for neuron 1 ##############
ax3.plot(sampling_time_points/1000,dep_mean_1,'-',color='#666666',label='from S to N1')
ax3.fill_between(sampling_time_points/1000,dep_mean_1-dep_std_1,dep_mean_1+dep_std_1,'-',color='#666666',alpha=0.2,label="")

ax3.plot(sampling_time_points/1000,dep_mean_2,'-',color='#47abd8',label='from E to N1')
ax3.fill_between(sampling_time_points/1000,dep_mean_2-dep_std_2,dep_mean_2+dep_std_2,color='#47abd8',alpha=0.2)


################## plot weights for neuron 2 ##############

ax2.plot(sampling_time_points/1000,ndep_mean_1,'-',color='#666666',label='from S to N2')
ax2.fill_between(sampling_time_points/1000,ndep_mean_1-ndep_std_1,ndep_mean_1+ndep_std_1,'-',color='#666666',alpha=0.2)

ax2.plot(sampling_time_points/1000,ndep_mean_2,'-',color='#47abd8',label='from E to N2')
...
ax3.set_ylabel(r"$\bar w$")
ax1.legend(bbox_to_anchor=(0.00,0.9,1.05, 0.1), loc=3,ncol=1, mode="expand", borderaxespad=0.,frameon=False)
ax2.legend(bbox_to_anchor=(0.0, 0.6,0.5, 0.1), loc=3,ncol=1, mode="expand", borderaxespad=0.,frameon=False)
ax3.legend(bbox_to_anchor=(0.0, 0.6,0.5, 0.1), loc=3,ncol=1, mode="expand", borderaxespad=0.,frameon=False)
